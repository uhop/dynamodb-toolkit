...
- **Adapter with hooks** — `prepare` / `revive` / `prepareKey` / `validateItem` / `checkConsistency` / `updateInput` / `prepareListInput`, single-op → `transactWriteItems` auto-upgrade, `Raw<T>` bypass marker, indirect-index second-hop for keys-only GSIs. Canned `stampCreatedAtISO()` / `stampCreatedAtEpoch()` prepare-hook builders.
...
- **Framework-agnostic REST core + `node:http` handler** — pure parsers/builders/policy plus a standard route pack ready to drop into `createServer`. Koa / Express / Fetch / Lambda adapters ship as subpath exports (see [Framework adapters](#framework-adapters)).
- **Table provisioning + CLI** — `dynamodb-toolkit/provisioning` ships `planTable` (read-only plan) and `ensureTable` (plan + apply) plus `verifyTable` (structured diff, `{throwOnMismatch}` optional) driven by the same Adapter declaration. Opt-in descriptor record detects drift `DescribeTable` can't see. `dynamodb-toolkit` CLI loads an ESM adapter module and runs `plan-table` / `ensure-table` / `verify-table`.
...
The bundled `dynamodb-toolkit/handler` is a pure `node:http` handler. Framework-specific bindings ship in the box as subpath exports — each adapter is a thin wrapper that translates its framework's request/response shape into the toolkit's `rest-core` parsers + standard route pack, and the core stays zero-dep at runtime (frameworks are duck-typed; `express` / `koa` appear only as type-level and test-side devDependencies). The wire contract (routes, query parameters, envelope keys, error mapping) is identical across all four.

| Sub-export                 | Runtime / framework                          | Notes                                                                                                                                                                                                                                                  |
| -------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `dynamodb-toolkit/koa`     | [Koa](https://koajs.com/) 2.x / 3.x          | `createKoaAdapter` — middleware                                                                                                                                                                                                                        |
| `dynamodb-toolkit/express` | [Express](https://expressjs.com/) 4.x / 5.x  | `createExpressAdapter` — middleware / Router                                                                                                                                                                                                           |
...
| `dynamodb-toolkit/express`      | `createExpressAdapter`                                                                                                                                                                                                                                                                                                                                    |
| `dynamodb-toolkit/koa`          | `createKoaAdapter`                                                                                                                                                                                                                                                                                                                                        |
| `dynamodb-toolkit/fetch`        | `createFetchAdapter`                                                                                                                                                                                                                                                                                                                                      |
| `dynamodb-toolkit/lambda`       | `createLambdaAdapter` (+ `dynamodb-toolkit/lambda/local.js`: `createNodeListener`, `createFetchBridge`)                                                                                                                                                                                                                                                   |
...
- **REST layer split** — `dynamodb-toolkit/rest-core` is framework-agnostic; `dynamodb-toolkit/handler` is the `node:http` adapter. Koa / Express / Fetch / Lambda bindings are subpath exports.
