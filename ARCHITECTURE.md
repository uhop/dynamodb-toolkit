...
| `express/` `koa/` `fetch/` `lambda/` | Framework adapters over rest-core + `matchRoute` — same wire contract as `handler/`, translated per framework. Frameworks are duck-typed at runtime (zero deps; `express` / `koa` / `aws-lambda` types are devDeps only). `lambda/local.js` adds `createNodeListener` / `createFetchBridge` local-debug bridges.            |
...
src/                    # Published code (ESM .js + .d.ts sidecars)
  ...
  koa/                  # Koa middleware adapter
  fetch/                # Fetch (Request → Response) adapter
  lambda/               # AWS Lambda adapter + local.js debug bridges
