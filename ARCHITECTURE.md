...
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `expressions/`  | `Update` / `Projection` / `Filter` / `Condition` / `KeyCondition` expression builders. Each allocates per-prefix counter-based aliases via the shared internal `allocator.js` (`makeAllocator`) so composing builders on the same `params` is collision-safe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `batch/`        | Chunked `BatchWriteItem` (25/call), single `TransactWriteItems` (100/call), `BatchGetItem` (100/call), `TransactGetItems`. `explainTransactionCancellation` maps `CancellationReasons` back to input descriptors. `backoff(from=50, to=20_000, finite?)` — AWS "stop around one minute" default; `RetryOptions` (`{backoff?, maxAttempts?}`) overrides it per call (`{options: {retry}}` sentinel), per mass helper (trailing arg), or per Adapter (`retry` option).                                                                                                                                                                                                                                                                                       |
| `mass/`         | Paginated reads (offset via `paginateList`, cursor via `cursorList` — native `LastEvaluatedKey`, constant cost per page), bulk-individual reads/writes/deletes (caller-supplied set), and list-op reads/writes/deletes (DB-produces set). Resumable list-op variants return `MassOpResult` `{processed, skipped, failed, conflicts, cursor?}`; the cursor envelope is versioned (`v: 1`). Naming: "List" means DB-produces-set; bulk-individual helpers drop it.                                                                                                                                                                                                                                                                                           |
| `paths/`        | Nested get/set/delete/patch on plain objects. Prototype-safe (rejects `__proto__` / `constructor` / `prototype` segments).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `rest-core/`    | Framework-agnostic REST primitives: parsers, builders, policy. DoS-gated (`parsePaging.maxOffset` 100k, `parseFields/Names.maxItems` 1000, `parseSearch.maxLength` 1024, `parseCursor` boundary validation, `validateWriteBody`, null-prototype accumulators, `confirmMassDelete` unscoped-delete guard). `parseFilter` (Option W): `?<op>-<field>=<value>`, ops `eq ne lt le gt ge in btw beg ct ex nx`. `parseMassOptions` (`?max-items` / `?resume`) + `buildMassResult` carry resumable mass ops on the wire.                                                                                                                                                                                                                                          |
...
| `http/`         | Framework adapters as ports over the shared neutral-result engine (`http/engine.js` — the route handlers + dispatch over rest-core + `matchRoute`; handlers return `{type, status, body?}` results of kind `json` / `text` / `empty` / `error` and never touch a socket). Each port owns only I/O translation and its framework's quirks (Express `headersSent → next`, Koa null-body coercion, fetch `onMiss`, Lambda envelopes). Same wire contract as `handler/`. Frameworks are duck-typed at runtime (zero deps; `express` / `koa` / `aws-lambda` types are devDeps only). Public subpaths stay short (`dynamodb-toolkit/express` → `src/http/express/`). `http/lambda/local.js` adds `createNodeListener` / `createFetchBridge` local-debug bridges. |
...
src/                    # Published code (ESM .js + .d.ts sidecars)
  ...
  http/                 # Framework adapters (public subpaths keep their short names)
    engine.js           #   Shared neutral-result engine — route handlers + dispatch; ports translate
    express/            #   createExpressAdapter — Express middleware port
    koa/                #   createKoaAdapter — Koa middleware port
    fetch/              #   createFetchAdapter — (Request) => Promise<Response> port
    lambda/             #   createLambdaAdapter port + local.js debug bridges
